...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CIN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("cinder:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CIN unit input in lnReqAmount, we generate the URI with CIN as unit (as defined in BIP21)
